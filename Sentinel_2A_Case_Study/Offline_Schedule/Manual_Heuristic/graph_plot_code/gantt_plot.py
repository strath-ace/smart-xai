...
def gantt_plot(memory_land_list_binary, daily, constraint_land_list, gantt_file):
    daily_land_list=[]
    ...
    for i in range(0, node_count_coord):
        ...
        elif daily_details[3] != 'Process_images' and (daily_details[3] != 'Matera' or daily_details[3] != 'Eumetsat' or daily_details[3] != 'PrudhoeBay' or daily_details[
            3] != 'Svalbard'):  # and any(e[3] == daily_details[3] for e in country_access(1, 'Dec', 2020, 'All')):
            Position = 'Land'
    ...
    dl = pd.DataFrame({'Task': ['Job_%i' for i in range(0, len(memory_land_list_binary))], 'Start': ['1970-01-01 ' + str(memory_land_list_binary[i][0]) for i in range(0, len(memory_land_list_binary))],
    ...
    dc = pd.DataFrame({'Task': ['Job_%i' for i in range(0, len(constraint_land_list))], 'Start': ['1970-01-01 ' + str(constraint_land_list[i][0]) for i in range(0, len(constraint_land_list))],
    ...
    fig1.update_layout(
        ...
        , yaxis=dict(
            ...
            , showticklabels=True
            , tickfont=dict(
                family="Arial", size=30, color='black'))
