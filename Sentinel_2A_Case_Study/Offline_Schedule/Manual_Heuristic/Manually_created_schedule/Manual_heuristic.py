...
def task_decision(occurrence_list, stations_summary, country_access_summary, file2):
    jobs_data = []
    ...
    while a < len(occurrence_list):
        ...
        if occurrence_list[a][3] == 'Penumbra Shade':
            ...
            penumbra_starttime = occurrence_list[a][0]
            penumbra_endtime = occurrence_list[a][1]
            ...
            for z in range(a, len(occurrence_list)):
                ...
                if penumbra_starttime <= occurrence_list[z][0] <= penumbra_endtime and any(
                        e[3] == occurrence_list[z][3] for e in stations_summary) and (occurrence_list[z][0] < occurrence_list[z][1]):
                ...
                elif penumbra_starttime <= occurrence_list[z][0] <= penumbra_endtime and any(e[3] == occurrence_list[z][3] for e in country_access_summary) and \
                        (occurrence_list[z][0] < occurrence_list[z][1]):
                    start_time = occurrence_list[z][0]
                    end_time = occurrence_list[z][1]
                    duration = occurrence_list[z][2]
                    job_action = 2
                ...
                elif penumbra_starttime <= occurrence_list[z][0] <= penumbra_endtime and \
                        occurrence_list[z][3] == 'Process_images' and (occurrence_list[z][0] < occurrence_list[z][1]):
                    start_time = occurrence_list[z][0]
                    end_time = occurrence_list[z][1]
                    duration = occurrence_list[z][2]
                    job_action = 2
        ...
        elif any(e[3] == occurrence_list[a][3] for e in stations_summary) and (prenum_repeat == 1) and (occurrence_list[a][0] < occurrence_list[a][1]):
            start_time = occurrence_list[a][0]
            end_time = occurrence_list[a][1]
            duration = occurrence_list[a][2]
            job_action = 4

        elif any(e[3] == occurrence_list[a][3] for e in country_access_summary) and (
                job_action != 0) and (prenum_repeat == 1) and (occurrence_list[a][0] < occurrence_list[a][1]):
            start_time = occurrence_list[a][0]
            end_time = occurrence_list[a][1]
            duration = occurrence_list[a][2]
            job_action = 1

        elif occurrence_list[a][3] == 'Process_images' and (prenum_repeat == 1) and (occurrence_list[a][0] < occurrence_list[a][1]):
            start_time = occurrence_list[a][0]
            end_time = occurrence_list[a][1]
            duration = occurrence_list[a][2]
            job_action = 2
...
def heuristic(path, day, month, year, country, time_interval):
    ...
    jobs_data = task_decision(occurrence_list, stations_summary, country_access_summary, file2)
