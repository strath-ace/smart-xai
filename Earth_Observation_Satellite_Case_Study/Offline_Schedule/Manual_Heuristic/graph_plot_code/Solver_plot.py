...
def solver_plot(day, path, solver_plot_image, image_mem, process_im_mem, onboard_mem, Max_pictures):
    ...
    for i in range(0, cp_count_coord):
        ...
        # Data from exported data format - start time| end time| action- optm for optimized|
        # memory | pictures in memory| processed images in memory| photos downloaded.
        constraint_land_list.append(
            [(dt.timedelta(seconds=(int(daily_cp_details[0])))), (dt.timedelta(seconds=int(daily_cp_details[1]))),
             action, daily_cp_details[4], float(daily_cp_details[5]), daily_cp_details[7],
    ...
    while i in range(0, len(constraint_land_list)):
        constraints.append(
            [constraint_land_list[i][0], constraint_land_list[i][1], constraint_land_list[i][2],
             constraint_land_list[i][3], constraint_land_list[i][4], constraint_land_list[i][5],
             constraint_land_list[i][6]])
        i += 1

    # Data collected from the solver results.
    ...
    # Data used to plot the number of images in memory at any 1 point/
    # "Time", "Pics", "Process", "Dump", 'Max_pics' from the solver.
    ...
    # Data used to plot the memory used at any 1 point/
    # "Time", "Memory", "Max_Memory" from the solver.
