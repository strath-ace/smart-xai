...
def plo_res(day, filename):

    # day = 3

    gantt_file1 = filename + '/Gantt_plot_infeasible' + str(day) + '.png'

    action_ap_path = filename + '/Attack_summary_ap' + str(day) + '.txt'
    ...
    # load ar attack file
    action_ar_path = filename + '/Attack_summary_ar' + str(day) + '.txt'
    ...
    action_ad_path = filename + '/Attack_summary_ad' + str(day) + '.txt'
    ...
    countap_ad = 0
    countap_ae = 0
    while i in range(1, count_ap_coord):
        ...
        if ap_data[3] == '-' and ap_data[5] == 'Exceeded':
            ...
            if ap_data[2] == '1':
                countap_ar = countap_ar + 1
            elif ap_data[2] == '2':
                countap_ad = countap_ad + 1
            elif ap_data[2] == '-1':
                countap_ae = countap_ad + 1
    ...
    for i in range(1, count_ar_coord):
        ...
        if ar_data[3] == '-' and ar_data[5] == 'Exceeded':
            ...
            if ar_data[2] == '0':
                countar_ap = countar_ap + 1
            elif ar_data[2] == '2':
                countar_ad = countar_ad + 1
            elif ar_data[2] == '-1':
                countar_ae = countar_ad + 1
    ...
    fig1.update_layout(bargap=0.5, bargroupgap=0.1, xaxis_range=[df.Start.min(), df.Finish.max()],
                       xaxis=dict(title= "Time", titlefont = dict (family='Arial', size=40, color='black'),showgrid=True, gridcolor='rgb(0,0,0)', side="bottom", tickmode='array', dtick="M1", tickformat="'%H:%M' \n", ticklabelmode="period", ticks="outside",
