#
# Copyright (C) 2022 Vaticle - Modified for research purposes at the University of Strathclyde
#
...
def load_data_into_typedb(input, session):
    ...
    print(items[2])
    for item in (items): # (items[3457,0],items[3]):  # 2
...
def satellite_template(Satellite_data):
    ...
    # ---------------idle-------------------------------
    graql_insert_query7 = 'insert $id isa idle '
    ...
    # graql_insert_query9 += 'insert $con(contained:$p, container:$m, contained:$im, contained:$d, contained:$id) isa contents; \n'
